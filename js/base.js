...
//  页面加载
function loadMainPage(ele, url, data) {
    url = contextPath+url;
...
+(function () {
    ...
    function newModal (obj){
        if (typeof obj == 'string'){// 只有dom元素 -- string

        }

        if (typeof obj == 'object'){// 提示框
...
//  ==================================================
+function () {
    $('body').on('click', '.i-checkbox-border', function () {
...
//  ==================================================
+function () {
//  #nav-sidle  #collapse-btn  左右滑动
    function toggleCollapse(){
        var open = $("#collapse-btn").hasClass("to-close");
        if(open){
            $('#nav-content').css({'padding-left': '0px'});
            $("#collapse-btn").removeClass("to-close").addClass("to-open");// 改变箭头方向，以及位置
            $('#nav-sidle').css('left', '-180px');
            $('#nav-head').css({'left':'0', 'width':'100%'});
        }else{
            $('#nav-content').css({'padding-left': '180px'});
            $('#nav-sidle').css('left', '0px');
            $('#nav-head').css({'left':'180px', 'width':'calc(100% - 180px)'});
...
//  右边小箭头
    var collapseBtn =
        '<div id="collapse-btn" class="to-close" onclick="xiaoi.toggleCollapse()">'+
        ...
        _item.click(function (){
            ...
            if(_item.parent().hasClass('panel')){
                if(_item.hasClass('flag-open')){
                    ...
                    _item.siblings('ul').slideToggle(200);
                    _item.children('i:last-child').css('transform', 'rotate(180deg)');
